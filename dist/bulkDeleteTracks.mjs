    ...
    try {
      ...
      if (trackUris.length > 0 && allPlaylists.length > 0) {
        ...
        if (currentPlaylistUri) {
          ...
          if (preferred) {
            try {
              const tracks = await getPlaylistTracks(preferred.uri, void 0, 500);
        ...
        if (!foundPlaylist) {
          for (const playlist of allPlaylists) {
            ...
            try {
              const tracks = await getPlaylistTracks(playlist.uri, void 0, 500);
              const trackUrisSet = new Set(tracks.map((t) => t.uri));
              const hasSelectedTrack = trackUris.some((uri) => trackUrisSet.has(uri));
              if (hasSelectedTrack) {
